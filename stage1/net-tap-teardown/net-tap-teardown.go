...
func main() {
	...
	brname := fmt.Sprintf("br%d", tapID)
	br, err := netlink.LinkByName(brname)
	if err != nil {
		fmt.Printf("Cannot find bridge: %v\n", err)
		os.Exit(5)
	}
	...
	links, err := netlink.LinkList()
	if err != nil {
		fmt.Printf("Cannot get link list: %v\n", err)
		os.Exit(6)
	}

	for _, link := range links {
		if link.Attrs().MasterIndex == brIndex {
			// restore original IP address
			mask := net.IPMask(netInfos[tapID].Mask)
			addr := &netlink.Addr{
				IPNet: &net.IPNet{
					IP:   netInfos[tapID].IP.To4(),
					Mask: mask},
				Label: ""}
			if err = netlink.AddrAdd(link, addr); err != nil {
				fmt.Printf("Cannot restore address: %v\n", err)
			}
		}
	}

	if err != netlink.LinkDel(br) {
		fmt.Printf("Cannot remove bridge: %v\n", err)
		os.Exit(6)
	}
}
