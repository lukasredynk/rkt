...
func TestCgroups(t *testing.T) {
	if testutils.IsKVM() {
		// systemd-nspawn is using host cgroups, capabilities and devices instead of lkvm which is
		// booting light VM with own kernel and cgroups. In the kvm flavor case this test is unnecessary
		t.Skipf("KVM flavor is running with its own cgroups, access to them doesn't affect the host")
	}
	ctx := testutils.NewRktRunCtx()
	defer ctx.Cleanup()
